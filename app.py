...
def call_gemini_to_extract_tasks(api_key: str, syllabus_text: str, model_name: str = "gemini-1.5-flash"):
...
"""
        ...
        # Guarantee JSON response using config (supported in gemini-1.5 models)
        generation_config = {}
...
def generate_ics(tasks_list) -> str:
    ...
    for task in tasks_list:
        # Check if due_date is correct YYYY-MM-DD
        ...
        # We set it as an all-day event on the due date
        # DTEND is set to the next day since in ICS, all-day events are [DTSTART, DTEND)
        try:
...
with st.sidebar:
    ...
    model_option = st.selectbox(
        ...
        index=0,
        help="기본값인 gemini-1.5-flash가 에러가 날 경우, 다른 모델로 변경하여 시도해 보세요."
    )
...
st.markdown("강의계획서 PDF만 업로드하면, AI가 마감 기한을 한눈에 볼 수 있도록 분석해 줍니다.")
...
with col1:
    ...
    st.subheader("📂 1단계: 강의계획서 PDF 업로드")
    uploaded_file = st.file_uploader("강의계획서 파일(.pdf)을 끌어다 놓거나 선택하세요.", type="pdf")
    
    # Logic when file is uploaded
...
# 6. Dashboard: Editable Table & Visual Schedule Grid
st.markdown("---")
st.subheader("📝 3단계: 추출된 일정 목록 확인 및 수정")

if st.session_state.tasks:
    # Convert tasks to pandas DataFrame for rendering and editing
    ...
    st.markdown("""
    💡 **엑셀처럼 사용법**:
    * 셀을 더블클릭하여 과목명, 과제명, 마감일을 **즉석에서 수정**할 수 있습니다.
    * 표 아래의 `+` 버튼을 눌러 **수동으로 일정을 추가**하거나, 행을 선택하고 `Del` 키를 눌러 **삭제**할 수 있습니다.
    """)
    ...
    # Filter out empty records and calculate D-Day
    valid_tasks = []
    ...
    if valid_tasks:
        ...
        # Display Tasks in custom cards in multi-column layout
        cols = st.columns(3)
    ...
    st.markdown("""
    ### 🔔 시작하기 가이드
    1. **데모 데이터 체험**: 좌측 사이드바에서 **'✨ 데모 데이터 로드'** 버튼을 누르시면 준비된 가상의 대학 과제 일정들이 표와 D-Day 요약으로 바로 채워집니다.
    2. **내 파일 분석**: 본인의 **Gemini API Key**를 좌측 사이드바에 입력한 후, 위의 **1단계**에 강의계획서 PDF를 업로드하여 파싱을 시작해 보세요.
    """)
    st.markdown('</div>', unsafe_allow_html=True)
