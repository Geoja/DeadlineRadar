...
def minify_html(html: str) -> str:
    """
    [핵심 수정] st.markdown은 마크다운 파서를 거치기 때문에,
    줄바꿈 + 4칸 이상 들여쓰기가 있는 HTML은 '코드블록'으로 해석되어 달력이 깨집니다.
    이 함수는 HTML 주석을 제거하고 줄바꿈/들여쓰기를 한 칸 공백으로 압축하여
    마크다운 파서가 HTML을 오해석하지 못하게 만듭니다.
    """
    html = re.sub(r"<!--.*?-->", "", html, flags=re.DOTALL)  # HTML 주석 제거
    html = re.sub(r"\n\s*", " ", html)                        # 줄바꿈+들여쓰기 압축
    return html.strip()
        ...
        for i, day in enumerate(week):
            if day == 0:
                ...
                if day in day_tasks:
                    ...
                    html += '<div style="display: flex; flex-direction: column; gap: 4px; overflow-y: auto; flex-grow: 1; padding-right: 2px; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.15) transparent;">'
                    for task in day_tasks[day]:
                        c_name = task.get("course", "기타")
                        color = color_map.get(c_name, "#6366f1")
                        title = str(task.get("task_name", "과제"))
                        ...
                        # 셀 라벨에 과목명 + 과제/시험명을 함께 표시 (예: "자료구조 기말고사 시험")
                        display_title = f"{c_name} {title}"
                        short_title = display_title if len(display_title) <= 16 else display_title[:14] + "..."
                        tooltip_text = f"[{c_name}] {title} | {full_desc}" if full_desc else f"[{c_name}] {title}"
                        # HTML 속성 안에 들어가므로 특수문자 이스케이프 (깨짐 방지)
                        ...
                        html += f'<div style="background: {color}; border: {border_style}; color: {text_color}; font-size: 0.7rem; padding: 2px 4px; border-radius: 4px; text-align: left; font-weight: {font_weight}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer;" title="{tooltip_text}">{prefix}{short_title}</div>'
                    html += '</div>'
                html += '</div>'
                
    html += "</div></div>"
    
    # [핵심 수정] 마크다운 파서가 들여쓰기/줄바꿈을 코드블록으로 오해석하지 않도록 압축하여 반환
    ...
    * 표 아래의 `+` 버튼을 눌러 **수동 일정을 추가**하거나, 행을 선택하고 `Del` 키를 눌러 **삭제**할 수 있습니다.
    ...
    if valid_tasks:
        ...
        for idx, (task, badge, days_left) in enumerate(sorted_tasks):
            ...
            with col_target:
                # [수정] 멀티라인 들여쓰기 HTML도 코드블록으로 깨질 수 있어 minify 적용
                card_html = f"""
                <div class="card">
                ...
                """
                st.markdown(minify_html(card_html), unsafe_allow_html=True)
    else:
