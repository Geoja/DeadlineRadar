...
def call_gemini_to_extract_tasks(api_key: str, syllabus_text: str, model_name: str = "gemini-3.5-flash"):
    ...
    try:
        ...
        # System instructions optimized for parsing syllabus deadlines
        prompt = f"""
전달된 대학교 강의계획서(Syllabus) 텍스트를 분석하여, 평가 항목(과제, 프로젝트, 시험, 토론, 퀴즈 등)의 마감 기한과 일정을 추출해주세요.

다음 규칙을 반드시 지켜주세요:
1. 각 과제별로 '과목명(course)', '과제/시험명(task_name)', '마감일(due_date)', '세부설명(description)'을 추출해야 합니다.
...
3. 만약 구체적인 날짜 대신 '14주차 제출', '15주차 시험' 등 주차만 나와 있는 경우, 2026학년도 1학기 개강일인 2026년 3월 2일(월요일)을 1주차 첫 날로 가정하여 날짜를 계산해주세요.
   ...
   - 주차 표기 시 마감 요일은 별도 명시가 없으면 해당 주차의 '일요일'로 설정합니다.
...
[
  {{
    "course": "SW프로그래밍의기초",
    "task_name": "과제 1: PRD 작성",
    "due_date": "2026-05-18",
    "description": "프로그램 개발 계획서(PRD) 작성 및 PDF 제출"
  }}
]
...
def draw_html_calendar(tasks_list, year, month) -> str:
    """Generates visual HTML/CSS monthly calendar grid representing academic deadlines."""
    ...
    # Generate distinct colors for courses dynamically to look harmonic
    ...
    html = f"""
    <div style="background: rgba(30, 41, 59, 0.25); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 16px; padding: 15px; margin-top: 15px; box-shadow: 0 4px 24px rgba(0,0,0,0.15);">
        <table style="width: 100%; border-collapse: collapse; font-family: 'Outfit', sans-serif; text-align: center; color: #f1f5f9;">
            <thead>
                <tr style="border-bottom: 2px solid rgba(255, 255, 255, 0.1);">
                    <th style="color: #f87171; padding: 12px; font-weight: 700; width: 14.28%;">일</th>
                    <th style="padding: 12px; font-weight: 700; width: 14.28%;">월</th>
                    <th style="padding: 12px; font-weight: 700; width: 14.28%;">화</th>
                    <th style="padding: 12px; font-weight: 700; width: 14.28%;">수</th>
                    <th style="padding: 12px; font-weight: 700; width: 14.28%;">목</th>
                    <th style="padding: 12px; font-weight: 700; width: 14.28%;">금</th>
                    <th style="color: #60a5fa; padding: 12px; font-weight: 700; width: 14.28%;">토</th>
                </tr>
            </thead>
            <tbody>
    """
    ...
    for week in month_matrix:
        ...
        for i, day in enumerate(week):
            if day == 0:
                html += '<td style="padding: 10px 4px; height: 100px; background: rgba(0, 0, 0, 0.12); vertical-align: top; border: 1px solid rgba(255, 255, 255, 0.03);"></td>'
            else:
                is_today = (today.year == year and today.month == month and today.day == day)
                td_style = "padding: 8px 4px; height: 100px; vertical-align: top; border: 1px solid rgba(255, 255, 255, 0.06); position: relative;"
                if is_today:
                    td_style += "background: rgba(99, 102, 241, 0.12); box-shadow: inset 0 0 10px rgba(99, 102, 241, 0.25);"
                ...
                html += f'<td style="{td_style}">'
                
                # Day label
                label_style = f"font-weight: 700; font-size: 0.9rem; color: {day_color}; display: inline-block; margin-bottom: 6px;"
                if is_today:
                    label_style += "background: #6366f1; color: white; border-radius: 50%; width: 22px; height: 22px; line-height: 22px; text-align: center;"
                
                html += f'<div style="{label_style}">{day}</div>'
                
                # Render tasks inside this day cell
                if day in day_tasks:
                    html += '<div style="display: flex; flex-direction: column; gap: 4px; overflow-y: auto; max-height: 70px;">'
                    for task in day_tasks[day]:
                        ...
                        html += f"""
                        <div style="background: {color}; color: white; font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; text-align: left; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer;" title="{tooltip_text}">
                            {short_title}
                        </div>
                        """
                    html += '</div>'
